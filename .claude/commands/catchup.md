...
gh run list -R "$R" --workflow="" --limit 4
```
...
This repo uses only the default GitHub labels (`bug`, `documentation`, `enhancement`, `duplicate`, `good first issue`, `help wanted`, `invalid`, `question`, `wontfix`) plus `from-review`. There is no priority/epic/area taxonomy, so group by label where it helps and otherwise by the requirement IDs the issues cite (`DK-FR-0NN`, `DK-NFR-0NN`) or the risk-register entries (`R-0NN`). The durable project state lives in `docs/` — implementation-plan (milestones), decision-log (ADRs), risk-register (open risks), release-checklist — read those before git log; they carry the why.
...
<!-- skill-templates: catchup 3f7f775 2026-07-25 -->
