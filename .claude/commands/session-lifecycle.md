...
5. **Prior-session state** — if a handoff note, session ledger, or queue file exists (no handoff-note/session-ledger convention exists in this repo yet — durable state instead lives in docs/implementation-plan.md (milestones), docs/decision-log.md (ADRs), and docs/risk-register.md (open risks); read those before git log), read the ledger's STATE header (not the full history) and the handoff TL;DR. `/catchup` is the component skill for reconstructing a prior session where installed.
...
1. **Convergence note** — write what shipped, what remains, and *why* each remaining item is blocked or deferred, into the closing chat message — this repo has no session ledger; if one of docs/implementation-plan.md, docs/decision-log.md, or docs/risk-register.md needs updating because of what shipped, do that as part of the change (AGENTS rule 13/14), not as a separate convergence note.
...
4. **Cleanup** — remove this session's worktrees (leave other sessions' worktrees alone), prune branches only after verifying each had a `MERGED` PR, stop dev daemons/servers started this session, restore any test-env mutations (quit any manually-launched debug build before ending (`killall DockKeeper` / `killall dockkeeper` as applicable) — this is a MenuBarExtra accessory app, so a stray debug instance keeps running in the background; no test-env files get mutated during testing).
