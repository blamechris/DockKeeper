...
Identified during review of PR #${SOURCE_PR}. Cite the requirement ID (`DK-FR-0NN`, `DK-NFR-0NN`, `DK-PRIV-001`) or risk-register entry (`R-0NN`) it relates to, where one applies.
...
# Complexity labels: this repo has no complexity taxonomy — the block below is
# inert here and stays only so a later `skill update` sees an unmodified shape.
