...
This repo squash-merges PRs, so `gh pr list --state merged --search "merged:>=<date>"` (or `--json mergedAt,number,title,labels`) enumerates what shipped. Commits on `main` are one per PR, so `git log --oneline <prev-tag>..HEAD` is an equally good cross-check.
...
There is no CHANGELOG.md and no conventional-commit prefixes to map from — categorize by reading the change. Use these sections, omitting empty ones: **Added**, **Changed**, **Fixed**, **Packaging** (signing, notarization, cask, release scripts — this project ships an installer, so packaging changes are user-facing), **Docs**.
Anchor entries to requirement IDs (`DK-FR-0NN`, `DK-NFR-0NN`) where one applies, and preserve evidence labels: something verified only in unit tests is not the same claim as something confirmed on hardware.
...
There is no changelog file. The rendered section is the **GitHub release body** — write it there via `gh release create` / `gh release edit`. README install instructions are updated separately (checklist §7).
