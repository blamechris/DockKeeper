...
```bash
# # macos-15 runner, Swift build + tests: a full run lands in about 1-2 minutes
...
- **Swift 6 strict-concurrency error** — actor isolation or `@MainActor` boundary violation. A real defect; fix the code, never loosen the setting.
- **No-networking-symbols gate** (`::error::networking symbol referenced in …`) — a genuine DK-NFR-002 violation: some new API or dependency pulled in `NSURLSession`/`CFNetwork`/`NWConnection`/`NWListener`/`CFSocket`. **Never silence this gate to go green** — find what introduced the symbol and remove it.
- **Test failure in `DockKeeperTests`** — the suite links only `DockKeeperCore`, so a failure here is pure-logic, reproducible locally with `swift test`.
- **`Scripts/build-app.sh` failure in the assembly step** — usually a missing resource (`AppIcon.icns`, `Info.plist`) or a codesign issue; CI signs ad-hoc, so a signing failure there is not an identity problem.
- **Stale SwiftPM cache / spurious link error** — the one genuinely transient class; a re-run is legitimate.
...
# gh workflow run CI --ref <branch>   # or push an empty commit: git commit --allow-empty
```
