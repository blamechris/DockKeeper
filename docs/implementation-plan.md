...
**Acceptance met (unit level):** 12 `@Test` cases including the sequential-incumbency invariant and a 27-combination sweep asserting **exactly one survivor** over every start-time shape ([InstanceGuardTests.swift](../Tests/DockKeeperTests/InstanceGuardTests.swift)); full suite **127 tests** green; `swift build` clean under Swift 6 strict concurrency. **Honest compromises:** unbundled `swift run` builds are deliberately unguarded (option 3's brick-the-launch risk is worse); detection is not atomic; the guard yields to the most *senior* instance, not the newest version; a pending `dockkeeper://` URL on a deflected launch is dropped (option 4 experimentally refuted). **Not yet run, therefore not CONFIRMED:** the assembled guard against a real double launch — the eleven-row matrix in [test-strategy.md](test-strategy.md) §3b (no hardware gate). **Open follow-ons:** (1) run §3b — item 1 no longer gates the shipped guard's fast-user-switching safety, which the [ADR-012 amendment](decision-log.md#adr-012-single-instance-guard-in-process-at-appinit-lsmultipleinstancesprohibited-withheld) made a unit-tested per-uid invariant; (2) `LSMultipleInstancesProhibited` — accept or WONTFIX on the result of item 1, recorded in ADR-012.

---

## M14 — Restore borrowed Dock auto-hide across process death (DK-FR-013, ADR-013) ✅ code complete (2026-08-17)

Not a parity item and not a feature: a **correctness defect in shipped behavior**, GitHub issue [#29](https://github.com/blamechris/DockKeeper/issues/29). M12's `weHidIt` flag is in-memory only, so any untrappable death while a capture hide was held — SIGKILL, Force Quit, a crash, `Scripts/run-app.sh`'s SIGTERM→SIGKILL escalation, or the kill at logout — left macOS Dock auto-hide **on** with nothing recording that DockKeeper put it there. The next launch reads the flag false with auto-hide on, and `decide` returns `none` from then on: correct per ADR-011's rules, and permanently wrong in effect, with **no in-app recovery**. Governed by **[ADR-013](decision-log.md#adr-013-borrowed-system-state-is-persisted-before-the-borrowing-write-and-reconciled-at-launch-termination-hooks-are-an-optimization-not-the-mechanism)**, which amends [ADR-011](decision-log.md#adr-011-hide-the-dock-during-screen-capture-via-a-private-screen-watcher-flag--dock-auto-hide-opt-in) on the two statements the defect falsified.

- ✅ `Settings.screenShareHideRecord` (**`DockKeeperCore`**) — one key, one JSON blob (`ScreenShareHideRecord`, `hiddenAt` only), the `preferredDisplayFingerprint` pattern so a single `set` is atomic and an undecodable value degrades to "no record", which is the safe answer everywhere. **Not registered** (absence is a real state), **not** in `externallyObservedKeys` (observing it would make every hide a `.settingsChanged` reconcile and falsify ADR-011's verified coordinator claim — there is a test), **no `settingsVersion` bump** (the hook is for reinterpreting existing keys).
- ✅ **Write-ahead / write-behind ordering** in `ScreenShareHider` — the record lands *before* `writeAutoHide(true)` and is cleared *after* a successful `writeAutoHide(false)`; a failed hide write clears the record again. The two stores cannot be made atomic, so the record is deliberately a **superset** of "this may be ours", and a launch that reads auto-hide already off discards it with **zero** Dock writes. Cost: one defaults write per hide and one per restore — per *capture session*, not per tick (DK-NFR-001).
- ✅ `ScreenShareHider.repair(record:currentAutoHide:capturing:featureActive:now:window:)` (**pure, `DockKeeperCore`**) → `{none, discard, adopt, restore}`, applied once per process by `AppState.init` **before** any enable/feature gating — a Dock stuck auto-hidden must be repaired even after the user turns the feature, or DockKeeper, off. Safety property asserted directly: never `restore`/`adopt` without observing auto-hide **on**. `adopt` exists because restoring during a live capture would show the Dock for up to one poll interval mid-share; `featureActive` gates it because adopting a hide nothing will restore re-arms the bug.
- ✅ **`decide` is unchanged** — its 8-row ADR-011 table is byte-identical (CONFIRMED by diff) and must stay passing, which is the proof obligation ADR-013 records for the ADR-011 contract. `repair` establishes the initial `(weHidIt, record)` pair; `decide` then runs the steady state.
- ✅ **Manual floor** — "Turn Off Dock Auto-Hide" in the menu (shown only while the Dock is auto-hiding and the feature is on) and the same "Turn Off Dock Auto-Hide" permanently in Preferences ▸ Advanced. Unconditional by design: the only recovery for users already poisoned by a pre-fix build, or whose record was lost. It is also what makes a **finite** 7-day attribution window affordable.
- ✅ **The repair is announced, never silent — on both acting branches** — one line through the surface the menu already has (`AppState.screenShareRepairMessage`), superseded by the next real screen-share transition, and ranked *below* the recovery `statusMessage` so a sticky note can never occlude a `Degraded` / `Not converging` message. `.adopt`'s disclosure is deferred to the restore that discharges it rather than dropped. A silent mutation of a global system preference is the one thing that would make the fix worse than the bug.
- ✅ **Termination hooks kept, and demoted** — `AppState.prepareForTermination()`, `applicationWillTerminate`, and `TerminationSignals` (`SIG_IGN` + `DispatchSourceSignal` for `SIGTERM`/`SIGINT`) make the trappable quit instant. They are a **latency optimization, not the mechanism**: the design is correct with zero termination handling.
- ✅ `--diagnostics` gained a read-only `Screen-share:` line (relative age, never a wall-clock stamp — state only, DK-PRIV-001 S2), following ADR-012's `Other instances:` precedent.
- ✅ **Not shipped:** a heartbeat, an owner pid with a liveness check, an `fsync`'d file in Application Support, `willPowerOffNotification`, a watchdog helper process, or blind restore-on-launch (ADR-013 options 2, 6–9, each with its reason).

**Acceptance met (unit level):** a 10-row pure `repair` rule table, a **48-combination** safety sweep, one-step convergence, the 7-day window asserted as a constant, and 18 port-driven integration cases covering both write orderings *asserted from inside the injected Dock write* (so a reversed-order mutant fails), the no-record launch that touches neither private port, the unreadable-auto-hide tick that mints nothing, the kill-then-relaunch repair, `.adopt` issuing no Dock write, both discard cases with zero writes, the record-preserving unreadable-symbol case, and manual recovery with and without a record — 28 new tests across `ScreenShareRepairTests`, `ScreenShareRepairIntegrationTests` and `ScreenShareTerminationTests` in [ScreenShareTests.swift](../Tests/DockKeeperTests/ScreenShareTests.swift). **Honest compromises:** the irreducible ambiguity between "nothing touched it" and "the user set it deliberately" is not resolved by any mechanism — no intent signal exists — so the design bounds exposure with `now − hiddenAt`, an over-estimate that errs toward *not* restoring, and accepts a bounded false-restore risk (R-014) against a status-quo bug that is strictly worse. Record durability is established against **process** death only; kernel panic and power loss are UNKNOWN and untestable, with the manual floor as their recovery. **Not yet run, therefore not CONFIRMED:** every on-device claim — the real `kill -9` → relaunch cycle, the logout path, the hook firing in the signed bundle, `SIGTERM` via `TerminationSignals`, the `.adopt` no-flash property, and the menu item for the poisoned population — the ten-row matrix in [test-strategy.md](test-strategy.md) §3c (no hardware gate). **Open follow-ons:** (1) the "lying UI" defect — when the user genuinely runs auto-hide the Advanced toggle reads on while the feature is by-design inert, with no status surface saying so; (2) the standing M6/M12 cell for whether `CGSIsScreenWatcherPresent` flips on a real capture, deliberately **not** a blocker here (the defect is reachable whenever the detector fires, including spuriously); (3) whether `CoreDockSetAutoHideEnabled` survives a process exiting microseconds after the call returns.
