...
Reshaped for v1.0.0 (2026-07-25): `notarize.sh` now takes **either** artifact — a `.app` is zipped with `ditto` for submission and the ticket is stapled onto the bundle, a `.dmg` behaves as before and prints the post-staple sha256 last — and refuses ad-hoc-signed input up front. `package-dmg.sh` gained the staple gate and a post-package ticket re-check. See the two-ticket note in [technical design §13](technical-design.md).
