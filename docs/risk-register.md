...
| R-012 | N | **Coexistence with an installed DockLock fights corrections.** Migrating users (the target audience of a replacement) will have DockLock installed, possibly running, during the switch — two agents correcting the Dock is the textbook oscillation scenario. Observed 2026-07-23: the dev rig itself has DockLock Lite installed (not running). | Medium | Medium | Oscillation guard already ships (6/60 s → `Error` + explain); PROPOSED: detect a running DockLock process and show a friendly "quit DockLock while DockKeeper is enabled" note; README migration section. | Open — new 2026-07-23 |
