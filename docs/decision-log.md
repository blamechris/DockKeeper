...
**Date / Status.** 2026-08-17 · **Accepted** — implemented in the same change (`InstanceGuard` pure core + unit tests, `SingleInstance` adapter, `DockKeeperApp.init()` wiring, `Diagnostics` shared flag set + `Other instances:`, `run-app.sh` quit-then-build-then-open). End-to-end verification is **outstanding**: the [test-strategy](test-strategy.md) manual matrix for DK-FR-012 must run before this is treated as validated, item 1 first, since it also gates the `LSMultipleInstancesProhibited` follow-on.
...
- **The Siri/Shortcuts voice line leads with the pause and drops the mechanism tail.** Asked out loud whether DockKeeper is on, "enabled" alone is the wrong answer while it is deliberately correcting nothing.
...
**Date / Status.** 2026-08-18 · **Accepted** — implemented as `PauseRecord`, `Settings.pauseRecord`, `RecoveryCoordinator.syncPauseRecord` hung off `notifyState()` with an injected `persistPause`, `AppState.resumeStalePauseIfNeeded`, `StatusSummary.pauseRecord` + `Paused:` line + voice line, and the `--diagnostics` `Paused:` line. Answers the design question [#36](https://github.com/blamechris/DockKeeper/issues/36) recorded rather than resolving it by reflex, and unblocks §3b row 4.
