...
- **A wedged-but-alive instance makes the app unstartable from the GUI.** It keeps a valid start time, so it outranks every newcomer, and `LSUIElement` leaves no Force Quit row to kill it with. `pkill -x DockKeeper` or the pid from `--diagnostics` is the recovery; the env-var escape hatch cannot be set for a Finder or login-item launch. Detecting unresponsiveness at `App.init()` is deliberately not attempted — it is not cheap, and a wrong answer would evict a healthy instance.
