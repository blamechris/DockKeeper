...
- [ ] Single-instance smoke test (ADR-012 / DK-FR-012), **UNKNOWN until run** ([test strategy §3b](test-strategy.md) has the full nine-row matrix, tracked in [#30](https://github.com/blamechris/DockKeeper/issues/30)). PR #28 shipped the guard without adding a §1 line, so it was under-gated relative to DK-FR-013; recording it here fixes that asymmetry rather than leaving it implicit. Row 1 (fast user switching) gates the **shipped** guard's per-session scoping, which is INFERRED, not just the `LSMultipleInstancesProhibited` follow-on.
