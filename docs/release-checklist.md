...
- [ ] Bottom-Dock guard smoke test (ADR-015 / DK-FR-014), **UNKNOWN until run** ([test strategy §3d](test-strategy.md) has the full **nine**-row matrix). Needs **two rigs, not one**: a two-display arrangement with **both bottom edges free** for rows 1–8, and — since #83 made clamping per-span — one display **stacked above another and overhanging it** for **row 9**, which is the only row whose failure mode is a trapped cursor rather than an unguarded Dock. The side-by-side rig cannot exercise row 9 at all: it has no shared span to cross. This gate exists because the same omission happened twice: PR #28 shipped DK-FR-012 with no §1 line, this checklist recorded that as an asymmetry to fix, and PR #60 then shipped DK-FR-014 the same way. The guard is the strongest candidate of any feature here for an on-device gate — it installs a system-wide event tap on mouse-move, it is Accessibility-gated, and its failure mode is a pointer that cannot reach the bottom of a screen.
