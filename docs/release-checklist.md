...
- [ ] CoreDock smoke test on the newest macOS point release (R-004: symbols resolve, live set works, fallback still engages when forced). **Partly done 2026-09-03 on 26.6.2**: `CoreDockGetRect`, `CoreDockGetAutoHideEnabled` and `CoreDockSetAutoHideEnabled` all resolved and the live set worked flicker-free in both directions ([session 5](hardware-matrix-results.md)); `CoreDockGetOrientationAndPinning` and the **forced-fallback** half were not exercised, so this box stays unchecked.
- [x] Screen-watcher smoke test on the newest macOS point release (R-004, ADR-011 / DK-FR-011): `CGSIsScreenWatcherPresent` resolves (else the toggle is disabled with a note); with the feature on, starting a real screen capture flips the flag and the Dock auto-hides, and stopping restores it — and a pre-existing user auto-hide is left untouched. **PASSED 2026-09-03 on macOS 26.6.2** ([session 5](hardware-matrix-results.md)): symbol resolves; a real `screencapture -V` recording flipped the flag in **< 0.5 s**, the Dock auto-hid ~2.5 s later and restored ~1 s after the capture ended; with the user's own auto-hide already ON the app wrote nothing at all. This was the DK-FR-011 true-case and had been UNKNOWN since the feature shipped 2026-07-23. **Measured on `screencapture` only** — QuickTime / Zoom / Screen Sharing.app stay INFERRED, on the grounds that the flag is system-wide rather than per-app.
- [ ] Crash-recovery smoke test on the newest macOS point release (ADR-013 / DK-FR-013), three cells — **the first two PASSED 2026-09-03 on 26.6.2** ([session 5](hardware-matrix-results.md)), the third still **UNKNOWN until run** ([test strategy §3c](test-strategy.md) has the full ten-row matrix):
  - [x] **Kill mid-capture.** With the feature on, start a real capture so the Dock auto-hides, then `kill -9` the app and relaunch it: auto-hide is back **off**, `--diagnostics` reports no record held, and the menu shows the "restored your Dock" note. **PASSED** — the defect condition was reproduced first (`SIGKILL` left auto-hide stuck ON with no owner and a record on disk); relaunch logged `Restored Dock auto-hide to off (repaired)` and **removed** the record key from the domain. The menu note itself was not inspected.
  - [x] **Kill mid-capture with the capture still running.** Same, but relaunch *while* the capture continues: the Dock does **not** flash — the repair adopts the existing hide and issues no Dock write — and stopping the capture restores auto-hide normally. **PASSED** — `Adopted a leftover screen-share Dock hide` with **no `Hid Dock` write**, auto-hide reading ON continuously across the kill and relaunch, then `Restored … (restored)` when the capture ended. The far-end *visual* is still unobserved: a local recording has no far end.
...
- [ ] Bottom-Dock guard smoke test (ADR-015 / DK-FR-014), **UNKNOWN until run** ([test strategy §3d](test-strategy.md) has the full **nine**-row matrix). Needs **two rigs, not one**, and §3d says which row needs which: a two-display arrangement with **both bottom edges free** for rows **1–4 and 6–8**, and one display **stacked above another and overhanging it** for rows **5 and 9**. Row 5 is the stacked-arrangement *refusal* and row 9 — new with #83 — is whether the pointer still crosses a shared span with per-span zones armed; **both need a shared span to exist**, which the side-by-side rig by definition does not have. Row 9's failure mode is a trapped cursor rather than an unguarded Dock, so it is the one that must not be skipped. It was **first run 2026-09-03 and CONFIRMED with a control** ([session 4](hardware-matrix-results.md)) — which retires it as an *open obligation* but **not** as a release gate: like row 1 it is re-run per macOS release, and the 2026-09-03 run was on 26.6.2. **Row 5 has still never been run**, and needs an edge blocked along its *whole* length, which an overhanging rig is not. This gate exists because the same omission happened twice: PR #28 shipped DK-FR-012 with no §1 line, this checklist recorded that as an asymmetry to fix, and PR #60 then shipped DK-FR-014 the same way. The guard is the strongest candidate of any feature here for an on-device gate — it installs a system-wide event tap on mouse-move, it is Accessibility-gated, and its failure mode is a pointer that cannot reach the bottom of a screen.
