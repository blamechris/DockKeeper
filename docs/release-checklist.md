...
- [ ] Bottom-Dock guard smoke test (ADR-015 / DK-FR-014), **UNKNOWN until run** ([test strategy §3d](test-strategy.md) has the full eight-row matrix). Needs a two-display rig with **both bottom edges free**. This gate exists because the same omission happened twice: PR #28 shipped DK-FR-012 with no §1 line, this checklist recorded that as an asymmetry to fix, and PR #60 then shipped DK-FR-014 the same way. The guard is the strongest candidate of any feature here for an on-device gate — it installs a system-wide event tap on mouse-move, it is Accessibility-gated, and its failure mode is a pointer that cannot reach the bottom of a screen.
  - [ ] **A real hand cannot complete the summon** on the non-preferred display, and the Dock does not migrate. This is the only INFERRED half of ADR-015; the clamp itself is CONFIRMED with a control.
  ...
  - **Owner decision for 0.9.3:** deferred, and shipped as a documented beta limit. Two displays exist — a MacBook Pro and a DELL G3223Q — but their **native arrangement is stacked** (the Dell sits flush above the laptop), which is precisely the geometry the guard refuses; the free-bottom-edge precondition needs a deliberate side-by-side rearrange, as the [spike](spikes/separate-spaces-pinning.md) did on 2026-08-27, and that has not been scheduled. Row 1 has never been run on any arrangement. Note the spike's own hand test is the **unguarded positive control** — it established that a real pointer *does* summon — which is the opposite of row 1, so it cannot be cited as row 1 having passed. The feature is opt-in and off by default, so no user carries an unverified mechanism unasked, and the CHANGELOG's *Known beta limits* says so in the user's own words.
