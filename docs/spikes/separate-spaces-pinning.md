...
**Date started:** 2026-07-23 · **Status:** Core question RESOLVED same day (→ ADR-009 shipped); bottom-mode (G1) **resolved 2026-08-27** — the clamp control ran on a free-bottom-edge rig and is ADR-015's Evidence; shipped opt-in in v0.9.3. What remains open is not the mechanism: the human-scale confirmation that a real hand cannot complete the summon was **CLOSED 2026-09-02** with a control ([test strategy §3d](../test-strategy.md) row 1, [hardware matrix session 3](../hardware-matrix-results.md)) — this line said otherwise until 2026-09-03. Open now is **§3d row 9**, whether the pointer still crosses a *shared* span with per-span zones armed (#83), which the side-by-side rig that closed row 1 had no shared span to test · **Drives:** ADR-008/009/015, implementation-plan M8
