...
3. ~~**G1** (bottom-mode lock)~~ — **closed 2026-08-27, not viable**; no mechanism exists that is reachable from outside the Dock process.
...
With G4+G5+G6 shipped, **G1 was the only remaining gap to every *shipping* DockLock capability — and it is now closed as not viable** (2026-08-27), so the honest position is that this one capability of theirs cannot be matched rather than has not been. G2/G3 close the premium tier; both were scoped as G1-dependent for bottom Docks and need rescoping to left/right only.
