...
With G4+G5+G6 shipped and **G1 shipped opt-in on 2026-08-29**, DockKeeper has an implementation for every *shipping* DockLock capability — by prevention rather than relocation, which is almost certainly what DockLock does too (INFERRED from constraint-fit, not reverse-engineered). G1's on-device confirmation is closed as of 2026-09-03 — the summon block on a free-edge rig and the per-span crossing on a stacked one, each with a control. Its *cost* (R-015) and §3d rows 3/5/6/7/8 remain open. G2/G3 close the premium tier.
