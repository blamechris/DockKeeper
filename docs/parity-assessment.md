...
3. **G1** (bottom-mode lock) — **reopened 2026-08-27**: prevention via event tap is confirmed; needs an ADR (Accessibility, opt-in) and a shared-bottom-edge safety gate. Relocation remains impossible; the feature must be framed as *stop the Dock leaving*, not *put it back*.
...
With G4+G5+G6 shipped, **G1 is the only remaining gap to every *shipping* DockLock capability, and as of 2026-08-27 it is viable** — by prevention rather than relocation. G2/G3 close the premium tier.
